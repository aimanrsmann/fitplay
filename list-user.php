...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['reject_booking'])) {
        // Cancel Booking
        $UserID = $_POST['booking_id'];
        $query = "DELETE FROM User WHERE UserID = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $UserID);

        if ($stmt->execute()) {
            echo "<script>alert('The selected user has been deleted!'); window.location.href='list-user.php';</script>";
        } else {
                                ...
                                <table class="min-w-[1500px] bg-white shadow-md mx-10 mt-5 rounded-lg">
                                    <thead>
                                        <tr class="bg-green-900 text-white">
                                            ...
                                            <th class="py-3 px-2 text-left">User ID</th>
                                            <th class="py-3 px-6 text-left">Email</th>
                                            <th class="py-3 px-6 text-left">Contact</th>
                                            <th class="py-3 px-3 text-left">Password</th>
                                            <th class="py-3 px-20 text-left">Action</th>
                                        ...
                                        if ($result->num_rows > 0):
                                            ...
                                            while ($row = $result->fetch_assoc()): ?>
                                                <tr class="border-b hover:bg-gray-100">
                                                    ...
                                                    <td class="py-3 px-6"><?php echo $row['UserID']; ?></td>
                                                    <td class="py-3 px-6"><?php echo $row['email']; ?></td>
                                                    <td class="py-3 px-6"><?php echo $row['contact']; ?></td>
                                                    <td class="py-3 px-6">*****</td>
                                                    <td class="py-3 px-6">
                                                        <a href="user-details.php?user_id=<?php echo $row['UserID']; ?>"
                                                            class="bg-blue-500 text-white px-4 py-2 rounded inline-block text-center hover:bg-blue-600">
                                                            Manage
                                                        </a>

                                                        <!-- Delete Button -->
                                                        <form id="reject-form-<?php echo $row['UserID']; ?>" method="POST" style="display:inline;">
                                                            <input type="hidden" name="booking_id" value="<?php echo $row['UserID']; ?>">
                                                            ...
                                                            <button type="button" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600"
                                                                onclick="confirmReject(<?php echo $row['UserID']; ?>)">
                                                                Delete
                                                            </button>
                                                        </form>
                                                    </td>
